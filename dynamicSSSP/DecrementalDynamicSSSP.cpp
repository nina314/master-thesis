...
: graph(graph), D(D), n(graph.size()), eps(eps)
{
    ...
    mes = MonotoneEStree(D, src, F);
    mes.initialize();
}

void DecrementalDynamicSSSP::initialize() {
...
void DecrementalDynamicSSSP::deleteEdge(int s, int d) {
        auto prev = matrix;
        for (int i = 1; i < p; i++) {
    ...
    vector<int> DecrementalDynamicSSSP::getDistances()
    {
